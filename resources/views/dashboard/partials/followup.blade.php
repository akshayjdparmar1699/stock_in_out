<div class="divide-y">
    @forelse ($followUpCustomers as $row)
        <div class="flex justify-between items-center px-5 py-3">
            <div>
                <div class="text-sm text-gray-800">{{ $row['customer']->name }}</div>
                <div class="text-xs text-gray-500">{{ __('Last purchase') }}: {{ $row['last_date']->format('d M Y') }}</div>
            </div>
    ...
    @empty
        <div class="px-5 py-6 text-sm text-gray-400">{{ __('All regular customers have purchased recently.') }}</div>
    @endforelse
</div>
<div class="px-5 py-3 border-t" @click="onClick($event)">{{ $followUpCustomers->onEachSide(0)->links() }}</div>
