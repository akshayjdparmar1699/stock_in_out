<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Invoice') }} {{ $invoice->invoice_number }}</h2>
    </x-slot>
    ...
    <div class="py-12" x-data="invoiceForm()">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('invoices.update', $invoice) }}" @submit="if (lines.length === 0) { alert('{{ __('Add at least one item.') }}'); $event.preventDefault(); }">
                        ...
                        <div class="flex items-center justify-between bg-gray-50 rounded-md px-4 py-3">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="text-right" x-show="(selectedCustomer.due || 0) != 0">
                                    <div class="text-xs text-gray-500">{{ __('Current balance') }}</div>
                ...
                {{-- Totals --}}
                <div class="bg-white shadow-sm rounded-lg p-6 mb-6">
                    <h3 class="font-medium text-gray-700 mb-4">{{ __('3. Totals') }}</h3>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <div>
                                <x-input-label for="invoice_date" :value="__('Invoice Date')" />
                                <x-text-input id="invoice_date" name="invoice_date" type="date" class="mt-1 block w-full" value="{{ $invoice->invoice_date->format('Y-m-d') }}" required />
                            </div>
                            <div>
                                <x-input-label for="notes" :value="__('Notes (optional)')" />
                                <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ $invoice->notes }}</textarea>
                            </div>
                            <p class="text-xs text-gray-400">{{ __('Payments already recorded on this invoice are not changed here — use "Record a Payment" on the invoice page for that.') }}</p>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between items-center text-sm font-semibold pt-1 border-t"
                                :class="(total - {{ (float) $invoice->paid_amount }}) > 0 ? 'text-red-600' : 'text-green-600'">
                                <span x-text="(total - {{ (float) $invoice->paid_amount }}) > 0 ? '{{ __('New Balance Due') }}' : '{{ __('Settled / In Credit') }}'"></span>
                                <span x-text="(total - {{ (float) $invoice->paid_amount }}) < 0
                                    ? '₹' + Math.abs(total - {{ (float) $invoice->paid_amount }}).toFixed(2) + ' CR'
                                    : '₹' + (total - {{ (float) $invoice->paid_amount }}).toFixed(2)"></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end">
                    <x-primary-button>{{ __('Update Bill') }}</x-primary-button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function invoiceForm() {
            return {
                ...
                nextLineId: {{ count($initialLines) + 1 }},

                init() {
                    this.lines = this.lines.map(line => {
                        const item = this.allItems.find(i => i.id === line.item_id) || {};
                        return {
                            ...line,
                            baseUnit: item.unit || line.unit,
